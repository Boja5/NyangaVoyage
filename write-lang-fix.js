...
// ============================================================
// FILE 1: lib/i18n.tsx — Language context
// ============================================================
...
// ============================================================
// FILE 2: components/Navbar.tsx — Shared navbar with lang toggle
// ============================================================
    ...
    <nav className="nv-nav">
      <div className="nv-nav-inner">
        ...
        <div className="nv-nav-links">
          <Link href="/search" className="nv-nav-link">
        ...
        <div className="nv-nav-right">
          <div className="nv-lang-toggle">
            <button
              className={'nv-lang-btn' + (lang === 'fr' ? ' active' : '')}
              onClick={() => setLang('fr')}
            >FR</button>
            <button
              className={'nv-lang-btn' + (lang === 'en' ? ' active' : '')}
              onClick={() => setLang('en')}
            >EN</button>
          </div>
...
// ============================================================
// FILE 3: app/layout.tsx — With LangProvider wrapping everything
// ============================================================
...
// ============================================================
// FILE 4: app/page.tsx — Uses useLang from context
// ============================================================
...
export default function HomePage() {
  ...
  const tabStyle = (active: boolean): React.CSSProperties => ({
    flex: 1, padding: '14px 20px', background: active ? 'var(--nv-bg-surface)' : 'var(--nv-gray-50)',
    ...
    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '-1.5px',
  })
  ...
  return (
    <div style={{ fontFamily: 'var(--nv-font-body)' }}>
            ...
            <div style={{ padding: '24px 28px' }}>
              {mode === 'bus' ? (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: '12px', alignItems: 'end' }}>
                  ...
                  <button className="nv-btn nv-btn-primary" onClick={handleBusSearch} style={{ height: '42px', paddingLeft: '24px', paddingRight: '24px' }}>{t.search.search} &rarr;</button>
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: '12px', alignItems: 'end' }}>
                  <div className="nv-form-group">
                  ...
                  <button className="nv-btn nv-btn-primary" onClick={handleTrainSearch} style={{ height: '42px', paddingLeft: '24px', paddingRight: '24px' }}>{t.search.search} &rarr;</button>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="nv-container">
          <div style={{ display:'flex', alignItems:'center', gap:'16px', flexWrap:'wrap' }}>
            ...
            {['Buca Voyages','Garanti Express','Vatican Express'].map(a=>(
              <div key={a} style={{ background:'var(--nv-green-50)', border:'1px solid var(--nv-green-200)', borderRadius:'var(--nv-radius-full)', padding:'6px 16px', fontSize:'13px', fontWeight:500, color:'var(--nv-green-800)' }}>{a}</div>
            ))}
            <div style={{ background:'var(--nv-gold-50)', border:'1px solid var(--nv-gold-200)', borderRadius:'var(--nv-radius-full)', padding:'6px 16px', fontSize:'13px', fontWeight:500, color:'var(--nv-gold-700)' }}>Camrail</div>
          </div>
        </div>
      </section>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'16px', maxWidth:'900px' }}>
            {[
              ...
              <div key={i} className="nv-card" style={{ padding:'24px' }}>
                <div style={{ width:'44px', height:'44px', borderRadius:'12px', background:f.bg, border:'1.5px solid '+f.border, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:'14px', fontFamily:'var(--nv-font-display)', fontSize:'20px', fontWeight:800, color:f.color }}>{f.num}</div>
      ...
      <section className="nv-section-sm" style={{ background:'var(--nv-bg-surface)', borderTop:'1.5px solid var(--nv-border)', borderBottom:'1.5px solid var(--nv-border)' }}>
        <div className="nv-container">
          <h2 style={{ fontFamily:'var(--nv-font-display)', fontSize:'22px', fontWeight:700, letterSpacing:'-0.02em', color:'var(--nv-gray-900)', marginBottom:'24px' }}>{t.busRoutes.title}</h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:'12px' }}>
            {BUS_ROUTES.map((r,i)=>(
              <div key={i} className="nv-card nv-card-hover" style={{ padding:'16px 20px', display:'flex', alignItems:'center', justifyContent:'space-between' }}
                onClick={()=>{ setBusFrom(r.from); setBusTo(r.to); setMode('bus'); window.scrollTo({top:0,behavior:'smooth'}) }}>
                <div>
                  <div style={{ fontFamily:'var(--nv-font-display)', fontSize:'15px', fontWeight:700, color:'var(--nv-gray-900)' }}>{r.from} <span style={{ color:'var(--nv-green-500)' }}>&rarr;</span> {r.to}</div>
      ...
      <section className="nv-section-sm" style={{ background:'var(--nv-bg-page)' }}>
        <div className="nv-container">
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'24px' }}>
            <h2 style={{ fontFamily:'var(--nv-font-display)', fontSize:'22px', fontWeight:700, letterSpacing:'-0.02em', color:'var(--nv-gray-900)' }}>{t.trainRoutes.title}</h2>
            ...
            {TRAIN_ROUTES.map((r,i)=>(
              <div key={i} className="nv-card nv-card-hover" style={{ padding:'20px 24px' }}
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px' }}>
                  <div>
                    <div style={{ fontFamily:'var(--nv-font-display)', fontSize:'18px', fontWeight:700, color:'var(--nv-gray-900)' }}>{r.from} <span style={{ color:'var(--nv-green-500)' }}>&rarr;</span> {r.to}</div>
                    <div style={{ fontSize:'12px', color:'var(--nv-text-secondary)', marginTop:'3px' }}>{r.km} km &middot; Depart {r.depart} &middot; Arrivee {r.arrive}</div>
                  </div>
      ...
      <section className="nv-section" style={{ background:'var(--nv-bg-surface)', borderTop:'1.5px solid var(--nv-border)' }}>
        <div className="nv-container">
          <h2 style={{ fontFamily:'var(--nv-font-display)', fontSize:'28px', fontWeight:700, letterSpacing:'-0.02em', color:'var(--nv-gray-900)', marginBottom:'36px', textAlign:'center' }}>{t.howItWorks.title}</h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'24px', maxWidth:'900px', margin:'0 auto' }}>
            {[
              ...
              <div key={i} style={{ textAlign:'center' }}>
                <div style={{ width:'56px', height:'56px', borderRadius:'50%', background:'var(--nv-green-600)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px', fontFamily:'var(--nv-font-display)', fontSize:'18px', fontWeight:800, color:'#fff' }}>{s.num}</div>
...
console.log('Written: lib/i18n.tsx');
